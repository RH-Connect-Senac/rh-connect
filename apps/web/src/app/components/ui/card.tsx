...
function Card({
  ...
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground rounded-2xl border border-border shadow-[0_1px_2px_rgba(15,27,45,0.06)]",
        paddingClassName,
        className,
      )}
      {...props}
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <h4
      ...
      className={cn("text-lg font-bold leading-[1.35] text-card-foreground", className)}
      {...props}
    />
  );
}
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <p
      ...
      className={cn("text-sm font-medium leading-[1.5] text-muted-foreground", className)}
      {...props}
    />
  );
}
